...
//firmware  info
//#define INTOROBOT_FIRMWARE_VER  "1.0.1"
#define INTOROBOT_FIRMWARE_LIB_VER  "2016050100"
#define INTOROBOT_API_VER       "v1"

//firmware update address
// #define INTOROBOT_FW_UPDATE_URL "/v1/bins/"
#define INTOROBOT_FW_UPDATE_URL "/v1/bin"
...
#define INTOROBOT_UPDATE_URL "112.124.117.64"
#define INTOROBOT_SERVER_DOMAIN "112.124.120.127"

//#define INTOROBOT_UPDATE_URL "192.168.0.77"
//#define INTOROBOT_SERVER_DOMAIN "192.168.0.77"
        ...
        if(md5_Object != NULL)
        {
            ...
            if(dtoken_Object != NULL)
            {
                ...
                if(true==firmwareupdate.st_firmware_down(domain.c_str(), param.c_str(),  md5_Object->valuestring))
                {
                    ...
                    intorobot_system_param.system_flags.boot_flag = 4;
...
void intorobot_init(void)
{
   ...
   //测试用于写入参数只需要执行一次即可
// #ifdef PRAM_BURN_ENABLE
//     test_write_pram();
//     MO_ERROR(("test_write_pram"));
// #endif

    //read intorobot_system_param
    loadSystemParams(&intorobot_system_param); //读取参数
    if(intorobot_system_param.system_flags.initparam_flag == 1) //初始化参数 保留密钥
    {
        MO_DEBUG(("init fac\r\n"));
        initFacSystemParams(&intorobot_system_param);
    }
    else if(intorobot_system_param.system_flags.initparam_flag == 2) //初始化所有参数
    {
        MO_DEBUG(("init all\r\n"));
