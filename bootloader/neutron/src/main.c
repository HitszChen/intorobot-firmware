...
extern system_params_t intorobot_system_params;  	//设备标识参数
...
int main(void)
{
    ...
    if(BOOTLOADER_VERSION != intorobot_system_params.system_flags.version)
    {
        intorobot_system_params.system_flags.version = BOOTLOADER_VERSION;
        Save_SystemFlags();
    }
...
#define TIMING_ALL_RESET_MODE    30000  //完全恢复出厂判断时间 清空密钥
    ...
    {
        switch(intorobot_system_params.system_flags.boot_flag)
        {
        case 0:
            START_APP_MODE = 1;
            break;
    ...
    /* /\* if((1 == START_APP_MODE)||(1 == DEFAULT_FIRMWARE_MODE)||(1 == FACTORY_RESET_MODE)||(1 == OTA_FIRMWARE_MODE)) *\/ */
    /* if((1 == DEFAULT_FIRMWARE_MODE)||(1 == FACTORY_RESET_MODE)||(1 == OTA_FIRMWARE_MODE)) */
    /* { */
    /*     GPIO_ESP8266_Init(); */
    /*     Esp8266_Reset(); */
    ...
    /* HAL_RTCEx_BKUPWrite(&RtcHandle, RTC_BKP_DR2, 0xFFFF); */
...
{
    Set_RGB_LED_Color(RGB_COLOR_BLUE);
    intorobot_system_params.system_flags.boot_flag = 0;
    Save_SystemFlags();
    USBD_CDC_Init();
    while (1)
    {
...
void Enter_Default_Firmware_Update_Mode(void)
{
    ...
    intorobot_system_params.system_flags.boot_flag = 1;
    Save_SystemFlags();
    DEFAULT_Flash_Reset();
    intorobot_system_params.system_flags.boot_flag = 0;
    Save_SystemFlags();
...
void Enter_Factory_Update_Mode(char type)
{
    ...
    Uart1_Init();
    ESP8266_Init();
    delay(1000); // delay for flash successfully, important

    if(type==0)
    {
        intorobot_system_params.system_flags.boot_flag = 3;
    }
    else
    {
        intorobot_system_params.system_flags.boot_flag = 5;
    }
    Save_SystemFlags();
    FACTORY_Flash_Reset();
    intorobot_system_params.system_flags.boot_flag = 0;
    if(type==0)
    {
        intorobot_system_params.system_flags.initparam_flag = 1;
    }
    else
    {
        intorobot_system_params.system_flags.initparam_flag = 2;
    }
    Save_SystemFlags();
}
...
void Enter_OTA_Firmware_Update_Mode(void)
{
    ...
    intorobot_system_params.system_flags.boot_flag = 4;
    Save_SystemFlags();
    OTA_Flash_Reset();
    intorobot_system_params.system_flags.boot_flag = 0;
    Save_SystemFlags();
