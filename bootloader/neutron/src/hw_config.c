...
system_params_t intorobot_system_params;  	//设备标识参数
...
void HAL_SYSTICK_Callback(void)
{
    ...
    else if(DEFAULT_FIRMWARE_MODE || OTA_FIRMWARE_MODE || FACTORY_RESET_MODE)
    {
        LED_Toggle(LED_RGB);
        TimingLED = 50;
    }
...
{
    loadSystemParams(&intorobot_system_params);
}

void Save_SystemFlags(void)
{
    saveSystemParams(&intorobot_system_params);
}

void Init_SystemFlags(void)
{
    initSystemParams(&intorobot_system_params);
}
